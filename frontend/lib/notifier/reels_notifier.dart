...
class ReelsNotifier extends StateNotifier<AsyncValue<List<Reel>>> {
  ...
  ReelsNotifier(this._repository, this._backend)
  ...
  void toggleLike(String reelId) {
    ...
    _likeDebounce[reelId] = Timer(const Duration(milliseconds: 400), () async {
      ...
      try {
        ...
        // Track with backend
        _backend.trackInteraction(
          contentId: reelId,
          contentType: 'REEL',
          interactionType: finalState ? 'LIKE' : 'UNLIKE',
        );
      } catch (e) {
  ...
  /// Track a reel view with watch time (called when user swipes away)
  void trackView(String reelId, {int watchSeconds = 0}) {
    _backend.trackInteraction(
      contentId: reelId,
      contentType: 'REEL',
      interactionType: watchSeconds < 2 ? 'SKIP' : 'VIEW',
